...
has [qw /ipaddress networkkbsread passwordenabled nic-traffictype volumeavailable allocationstate dhcpprovider
     passwordenabled dns1 dns2 internaldns1 internaldns2 /] => ( is => 'rw', isa => 'Int', default => 17 );
...
has [qw /hostname isodisplaytext user-timezone displaytext description timezone guestcidraddress
     /] => ( is => 'rw', isa => 'Int', default => 20 );
...
has [qw/id accountid domainid groupid guestosid hostid isoid templateid jobid rootdeviceid serviceofferingid sourcetemplateid
     ...
     offeringid jobinstanceid cmd/] => ( is => 'rw', isa => 'Int', default => 38 );

has [qw /user-apikey user-secretkey apikey secretkey jobresult/] => ( is => 'rw', isa => 'Int', default => 88 );
